...
for model in ['deepseek', 'magicoder', 'llama', 'gpt', 'gemma']:
	file_to_load = os.path.join('..', '..', 'data', 'classeval', 'raw', f'results_ClassEval_{model}_eval.json')
	with open(file_to_load, 'r') as f:
	    res_model_list.append(json.load(f))

dataset_to_load = os.path.join('..', '..', 'data', 'classeval', 'prompts_generated_ClassEval.csv')
...
for p, model in enumerate(['deepseek', 'magicoder', 'llama', 'gpt', 'gemma']):
 file_to_save = f'results_ClassEval_{model}_sim.json'
 avg_dict = {}
 for ind_key in prompt.index.values:
...
with open(os.path.join('..', '..', 'data', 'classeval', 'sim', file_to_save), 'w') as f:
